...
class ContextualRangeFilterAssignmentForm extends SystemConfigFormBase {
        ...
        if (!empty($display['display_options']['arguments'])) {
          ...
          foreach ($display['display_options']['arguments'] as $contextual_filter) {
            ...
            if ($is_date_handler || $is_numeric_handler || $is_string_handler) {
              ...
              // There doesn't seem to be an easy human-readible alternative in D8...
              ...
              if ($is_date_handler) {
                $this->_addToRangeFields($range_fields['date_field_names'][$machine_name], $title, $view_name);
              }
              elseif ($is_numeric_handler) {
                $this->_addToRangeFields($range_fields['numeric_field_names'][$machine_name], $title, $view_name);
              }
              elseif ($is_string_handler) {
                $this->_addToRangeFields($range_fields['string_field_names'][$machine_name], $title, $view_name);
              }
            }
          }
        }
   ...
   */
  private function _addToRangeFields(&$range_field_view_names, $title, $view_name) {
